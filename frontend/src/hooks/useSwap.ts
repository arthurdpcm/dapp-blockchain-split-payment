...
export function useSwap() {
  ...
  const estimateGasForSwap = useCallback(
    async ({
      tokenInAddress,
      amountIn,
      tokenInDecimals,
      ...
      try {
        ...
        const splitPaymentContract = new ethers.Contract(
          contractAddresses.SplitPayment,
          SPLIT_PAYMENT_ABI.abi,
          provider
        );
        ...
        if (currentAllowance < amountToSwap) {
          approveGas = await tokenContract.approve.estimateGas(
            contractAddresses.SplitPayment,
            amountToSwap,
            { from: account }
          );
  ...
  const executeSwap = useCallback(
    async ({
      ...
      try {
        ...
        const errorMessage =
          err.reason || err.data?.message || err.message || 'Ocorreu um erro desconhecido.';
